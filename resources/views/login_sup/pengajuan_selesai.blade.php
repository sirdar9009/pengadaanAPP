...
<div class="wrapper">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="../../index3.html" class="brand-link">
      ...
      <span class="brand-text font-weight-light">HALAMAN SUPLIER</span>
    </a>
                    ...
                    <tr class="text-center">
                      ...
                      <td>
                        ...
                        @if($p['status'] == 2)
                          ...
                          <hr>
                          @if($p['laporan']=="")
                          <form method="post" action="/laporanPengajuan" enctype="multipart/form-data">
                            {{csrf_field()}}
                            <input type="hidden" name="id_pengajuan" id="id_pengajuan" value="{{$p['id_pengajuan']}}">
                            <label  for="laporan" class=" btn btn-block btn-outline-info btn-flat">Pilih Laporan Pengajuan</label>
                            <input type="file" name="laporan" id="laporan" class="form-control" style="display:none" accept="application/pdf">
                            <button type="submit" class="btn btn-secondary">Upload</button>
                          </form>
                          @else
                          <a href="{{asset(Storage::url($p['laporan']))}}" class="btn btn-warning"><i class="fa fa-eye"></i> Lihat Laporan</a>
                          @endif
                        @endif
                        @if($p['status'] == 3)
                          Telah Selesai<hr>
